...
def skipped_urls():
    ...
    urllist = pd.read_csv(config.get('input-data', 'filepath'),
    ...
    argus_chunks = [script_dir_edit + "/chunks/" + f for f in os.listdir(
        script_dir_edit + "/chunks") if f.startswith('ARGUS_chunk')]
    ...
    print("Number of non-scraped URLs: ",
          len(list(set(initial_urls) - set(scraped_urls))))

    # write skipped URLs to new list for further scraping
    new_urllist = config.get(
        'input-data', 'filepath').split(".")[0] + "_skipped_urls.txt"
    urllist[urllist[url_column].isin(list(set(initial_urls) - set(scraped_urls)))].to_csv(
        new_urllist, sep="\t", encoding="utf-8", index=False)
