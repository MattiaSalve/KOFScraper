...
def aggregate_webpages(filepath=None):
    try:
        ...
        data = pd.read_csv(filename, encoding="utf-8", sep="\t",
                           on_bad_lines="skip", chunksize=25000)
        outputfile = open(filename.split(
            ".")[0] + "_aggregated.csv", "w", encoding='utf-8')
        outputfile.write("ID" + "\t" + "dl_rank" + "\t" + "dl_slot" + "\t" + "error" + "\t" + "redirect" + "\t" +
        ...
        for chunk in data:
            ...
            for i, text in enumerate(texts):
                ...
                if dl_slots[i] != dl_slot:
                    output = output + "\t".join([str(x) for x in [ID, dl_rank, dl_slot, error, redirect,
                                                start_page, title, keywords, description, website_text, timestamp]]) + "\n"
                    dl_slot = dl_slots[i]
                    website_text = text
                    dl_rank = dl_ranks[i]
                    ID = IDs[i]
