...
def start_crawl(scraping_type):
    ...
    if len(script_dir.split(".")) > 1:
        ...
        # read URL file
        data = pd.read_csv(
            config.get("input-data", "filepath"),
            delimiter=config.get("input-data", "delimiter"),
            encoding=config.get("input-data", "encoding"),
            index_col=config.get("input-data", "ID"),
            on_bad_lines="skip",
            engine="python",
        )
        print(data)
        ...
        if config.get("spider-settings", "language") == "None":
            ...
            language_ISOs = (
                ISO_codes.loc[ISO_codes["language"] == language][
                    ["ISO1", "ISO2", "ISO3"]
                ]
                .iloc[0]
                .tolist()
            )
            language_ISOs = "{},{},{}".format(
                language_ISOs[0], language_ISOs[1], language_ISOs[2]
            )
        ...
        if scraping_type == "normal":
            ...
            for chunk in np.array_split(data, n_url_chunks):
                chunk.to_csv(
                    script_dir_edit + "/chunks/url_chunk_p" + str(p) + ".csv",
                    sep="\t",
                    encoding="utf-8",
                )
                print(len(chunk))
            ...
            for p in range(1, n_url_chunks + 1):
                ...
                if config.get("spider-settings", "spider") == "dual":
                    subprocess.run(
                        "curl http://localhost:6800/schedule.json -d project=ARGUS -d spider=dualspider -d url_chunk={} -d limit={} -d ID={} -d url_col={} -d language={} -d setting=LOG_LEVEL={} -d prefer_short_urls={} -d pdfscrape={}".format(
                            url_chunk,
                            config.get("spider-settings", "limit"),
                            config.get("input-data", "ID"),
                            config.get("input-data", "url"),
                            language_ISOs,
                            config.get("spider-settings", "log_level"),
                            config.get("spider-settings", "prefer_short_urls"),
                            config.get("spider-settings", "pdfscrape"),
                        )
                    )
                # schedule webarchive spider
                elif config.get("spider-settings", "spider") == "webarchive":
                    subprocess.run(
                        "curl http://localhost:6800/schedule.json -d project=ARGUS -d spider=webarchive -d url_chunk={} -d limit={} -d ID={} -d url_col={} -d language={} -d setting=LOG_LEVEL={} -d prefer_short_urls={} -d date={}".format(
                            url_chunk,
                            config.get("spider-settings", "limit"),
                            config.get("input-data", "ID"),
                            config.get("input-data", "url"),
                            ...
                            config.get("spider-settings", "log_level"),
                            config.get("spider-settings", "prefer_short_urls"),
                            config.get("spider-settings", "date"),
                        )
                    )
        ...
        if scraping_type == "skipped":
            ...
            for chunk in np.array_split(data, n_url_chunks):
                chunk.to_csv(
                    script_dir_edit + "/chunks/url_chunk_p" + str(p) + "-skipped.csv",
                    sep="\t",
                    encoding="utf-8",
                )
                p += 1
